...
from admin import *
from models import Users, MyRole
    ...
    if request.method.__eq__('POST'):
        ...
        try:
            if password.strip() == confirm.strip():
                utils.add_user(fullname=name, username=username, password=password, phone=phone, email=email)
                return redirect(url_for('normaluser_login'))
            else:
...
def complete_registration():
    try:
        ...
        email = s.loads(token, salt="email-verification", max_age=500)  # max_age: milliseconds
    ...
    if current_user.is_authenticated:
        ...
        if request.method == 'POST':
            try:
                ...
                err_msg = "System error"
        return render_template("shop-standart-forms.html", err_msg=err_msg, **context)
    return redirect("/")
...
def normaluser_change_password():
    ...
    if request.method == 'POST':
        try:
            ...
            err_msg = "System error"
    return render_template("change-password.html", err_msg=err_msg, **context)
...
def detail():
    ...
    return render_template('item-detail.html',
                           ...
                           similar_products=similar_products, **context)
...
def contact():
    ...
    return render_template("page-contacts.html", **context)
...
def careers():
    ...
    return render_template("page-careers.html", **context)
...
def cart():
    ...
    if current_user.is_authenticated:
        return render_template("shop-shopping-cart.html", **context)
    return redirect("/")
...
def checkout():
    ...
    if current_user.is_authenticated:
        ...
        return render_template("shop-checkout.html", allship=shipping, **context)
    return redirect("/")
...
def home():
    ...
    return render_template('home.html',
                           mostpupular_products=mostpupular_products, **context)
