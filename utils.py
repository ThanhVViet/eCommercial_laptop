...
from models import *
from __init__ import app, db, mail, s, logger
...
def add_user(fullname, username, password, phone, email):
    get_user = Users.query.filter(Users.username == username).all()
    if not get_user:
        user = Users(name=fullname.strip(), active=0, username=username.strip(), password=password.strip(),
                     phone=phone.strip()
                     , email=email)
        db.session.add(user)
        try:
...
#     get_user = Users.query.filter(Users.username == username).all()
#     if not get_user:
#         passwordCode = str(hashlib.md5(password.strip().encode('utf-8')).hexdigest())
...
def email_verification(email):
    ...
    try:
        ...
        logger.error(f"Email verification error: {ex}")
        print(ex)
        return False
